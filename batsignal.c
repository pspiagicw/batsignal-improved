...
static char *icon = "/home/pspiagicw/.config/dunst/battery.png";
static char *lowicon = "/home/pspiagicw/.config/dunst/battery-low.png";
static char *charicon = "/home/pspiagicw/.config/dunst/battery-charging.png";
static char *fullicon = "/home/pspiagicw/.config/dunst/battery-full.png";
static char *disconnecticon = "/home/pspiagicw/.config/dunst/battery-disconnect.png";
...
void notify(char *msg, NotifyUrgency urgency , char *iconpath)
{
  char body[20];
  sprintf(body, "Battery level: %u%%", battery_level);

  if (msg[0] != '\0' && notify_init(appname)) {
    NotifyNotification *notification = notify_notification_new(msg, body, iconpath);
...
int main(int argc, char *argv[])
{
  ...
  for(;;) {
	...
	charging_or_not = 0;
	notify(disconnectedmsg, NOTIFY_URGENCY_NORMAL,disconnecticon);
      }
      if (danger && battery_level <= danger && battery_state != STATE_DANGER) {
        ...
        notify(criticalmsg, NOTIFY_URGENCY_CRITICAL,icon);
	...
	charging_or_not = 1;
        if (full && battery_level >= full && battery_state != STATE_FULL) {
            ...
            notify(fullmsg, NOTIFY_URGENCY_NORMAL,fullicon);
        }
    }
